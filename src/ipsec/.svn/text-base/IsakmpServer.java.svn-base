...
public class IsakmpServer {
	...
	public static final String msg = "123456789";
	...
	public void run() {
		try {
			...
			IsakmpHeader receiveHeader = null;
			...
			System.out.println("\nreceived from "+pkt.getAddress()+":"+pkt.getPort()+"\n");
			src_addr = pkt.getSocketAddress();
			...
			// パケット保存して保存したパケットから読み込むかどうか
			//			this.saveByteStream(buf);
			//			buf = this.readByteStream("C:/Users/Nakamiri/Java_project/ipsec_dev/hoge.txt");

			receiveHeader = new IsakmpHeader(buf);

			long rnd = (long) (Math.random() * Long.MAX_VALUE);
			String seed = sock.getLocalAddress().toString() + sock.getLocalPort() + pkt.getAddress() + pkt.getPort() + rnd + System.currentTimeMillis();
			String res_cookie = MD5.digestMd5(seed).substring(0, 16);
			byte[] res_cookie_byte = BinaryHexConverter.HexStringToBytes(res_cookie);
			sendSaParamPkt(receiveHeader, res_cookie_byte);
			...
			DatagramPacket pkt4 = new DatagramPacket(buf4, buf4.length);
			sock.receive(pkt4);
			System.out.println("\nreceived from "+pkt.getAddress()+":"+pkt.getPort()+"\n");

			// エラーチェック
			if (checkMessage(buf4)) {
				return;
			}

			IsakmpHeader quick_sa = new IsakmpHeader(buf4, key_manager, decode);
			Vector<Payload> v = quick_sa.getPayload(Payload.NONCE_PAYLOAD);
			quick_init_nonce = (NoncePayload) v.firstElement();
